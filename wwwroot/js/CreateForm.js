...
async function populateOptions(){
  try{//http://192.168.1.131:5034/api/room-type?hotelId=1
    const roomType = await (await fetch(`http://192.168.1.131:5034/api/room-type?hotelId=${hotelId}`)).json()
    console.log('roomType: ', roomType);
    const cancelPolicy = await (await fetch("https://api2-pnv.bluejaypos.vn/api/cancel-policy")).json()
    console.log('cancelPolicy: ', cancelPolicy);
    const paymentContraint = await (await fetch("http://192.168.1.131:5034/api/payment-constraint")).json()
    console.log('fetch paymentContraint: ', paymentContraint);
    const additional = await (await fetch(`http://192.168.1.131:5034/api/additional?hotelId=${hotelId}`)).json()
